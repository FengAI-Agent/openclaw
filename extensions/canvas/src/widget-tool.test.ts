...
async function executeWidget(params: {
  ...
  const preview = extractCanvasFromText(text, "show_widget");
  if (!preview?.viewId || !preview.url) {
...
describe("show_widget", () => {
  ...
  it("wraps SVG widgets with the sandbox CSP and SVG layout", async () => {
    const stateDir = await createStateDir();
    const { preview, text } = await executeWidget({
      stateDir,
      title: "<Status>",
      widgetCode: '  <SvG viewBox="0 0 10 10"><circle r="4" /></SvG>  ',
    });

    expect(preview.viewId).toMatch(/^cv_[a-f0-9]{32}$/);
    expect(preview.url).toBe(`/__openclaw__/canvas/documents/${preview.viewId}/index.html`);
    expect(JSON.parse(text)).toMatchObject({
      kind: "canvas",
      presentation: { target: "assistant_message", title: "<Status>", sandbox: "scripts" },
      text: `Widget hosted at ${preview.url}`,
    });
    expect(preview.sandbox).toBe("scripts");
    const html = await readFile(
      path.join(resolveCanvasDocumentDir(preview.viewId, { stateDir }), "index.html"),
      "utf8",
    );
    ...
    const manifest = JSON.parse(
      await readFile(
        path.join(resolveCanvasDocumentDir(preview.viewId, { stateDir }), "manifest.json"),
        "utf8",
      ),
  ...
  it("wraps HTML fragments without SVG layout", async () => {
    const stateDir = await createStateDir();
    const { preview } = await executeWidget({
      stateDir,
  ...
  it("uses opaque ids and evicts the oldest widget within a session scope", async () => {
    ...
    await expect(
      access(resolveCanvasDocumentDir(first.preview.viewId, { stateDir })),
    ).rejects.toThrow();
